				...
				<div class="main-content-inner">
					<div class="breadcrumbs ace-save-state" id="breadcrumbs">
						<ul class="breadcrumb">
							...
							<li class="active">Listar planes funerarios</li>
						</ul><!-- /.breadcrumb -->
					...
					<!-- Modal content-->
						<div class="modal-content">
							...
							<div class="modal-body">
								...
								<div id="caption-img" class="caption text-center">
						        </div>
								<br>
							</div>
							<div class="modal-footer">
...
<![endif]-->
		...
		<script type="text/javascript">
			...
			function format ( d ) {
				...
				if ((imgSrc.length > 0))
					imagenHtml = '<img class="profile-picture img-prev" idplan="'+d.id+'" height="125px" src="'+imgSrc+'">';
				else
					imagenHtml 	= '<i class="fa fa-5x fa-file-image-o profile-picture" aria-hidden="true" style="min-width:100%;color:darkgray"></i>';
				// idSucursal 		= d.id;
				htmlReturn = '<div class="row">'+
							 '	<div class="col-xs-2 text-center">'+
									imagenHtml+
							 '	</div>';
				htmlReturn +='	<div class="col-xs-8" style="padding-left:0px">'+
			...
			$(document).ready(function()
			{
				...
				$(document).on('click','#btnEliminarModal',function()
				{
			        ...
			        {
			            method: "POST",
			            url:"assets/ajax/eliminarPlan.php",
				...
				$(document).on("click","img.img-prev",function()
				{
					idCliente = $(this).attr("idplan");
					$("#modal-img").modal();
					$.ajax(
			        {
			            method: "POST",
			            url:"assets/ajax/infoImagenPlan.php",
			        ...
			        .done(function(p)
			        {
						if (p.status == 1)
						{
							$("#imgSrc").html(p.imgSrc);
							$("#caption-img").html(p.caption);
						}
						else
						{
				...
				{
					$("#imgSrc").html(cargarSpinner+" Cargando...");
					// $("#imgSrc").empty();
					$("#caption-img").empty();
			...
			jQuery(function($) {
				...
				myTable = $('#dynamic-table').DataTable( {
			        ...
			        "ajax":
					{
						"url":"assets/ajax/listarPlanes.JSON.php",
						"complete": function()
						{
					...
					'createdRow': function( row, data, dataIndex ) {
						$(row).attr('id', data.id);
					     $(row).attr('id-sucursal', data.idSucursal);
					 },
			        "order": 		[[0, 'asc']]
			    } );
				...
				$(document).on('click', 'td.details-control', function ()
				{
					...
					else
					{
						...
						idSucursal = tr.attr("id-sucursal");
						$(this).html(cargarSpinner);
						infoRow = $.ajax(
						{
							method: "POST",
							url:"assets/ajax/obtenerRowDet_plan.php",
							dataType:'JSON',
							data: {idCliente:idCliente,idSucursal:idSucursal}
						})
						.always(function(p)
						{
