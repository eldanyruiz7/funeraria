...
<![endif]-->
		...
		<script type="text/javascript">
			...
			function format ( d ) {
				imgSrc = d.imgSrc;
				if ((imgSrc.length > 0))
					imagenHtml = '<img class="profile-picture img-prev" idproducto="'+d.id+'" height="100%" src="'+imgSrc+'">';
				else
					imagenHtml 	= '<i class="fa fa-5x fa-file-image-o profile-picture" aria-hidden="true"></i>';
				// idSucursal 		= d.id;
				descripcion 	= d.descripcion;
				fechaCreacion 	= d.fechaCreacion;
				listaExitencias = d.existenciasXsuc;
				// nombreSucursal 	= d.nombreSucursal;
				// alert("Logrado");
				console.log(d);
				return '<table class="table table-striped table-bordered table-hover">'+
							'<tr>'+
								'<td rowspan="4" height="125px" style="color:darkgray;text-align:center">'+
								imagenHtml+
								'</td>'+
			...
			$(document).ready(function()
			{
				...
				{
					idCliente = $(this).attr("idproducto");
					$("#modal-img").modal();
					$.ajax(
			        {
			            method: "POST",
			            url:"assets/ajax/infoImagenProducto.php",
			...
			jQuery(function($) {
				...
				myTable = $('#dynamic-table').DataTable( {
			        ...
			        "ajax":
					{
						"url":"assets/ajax/listarProductos.JSON.php",
						"complete": function()
						{
				...
				$(document).on('click', 'td.details-control', function ()
				{
					...
					else
					{
						...
						{
							method: "POST",
							url:"assets/ajax/infoProducto.php",
							dataType:'JSON',
							data: {idCliente:idCliente}
						})
						.always(function(p)
