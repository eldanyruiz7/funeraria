<?php
    ...
    else
    {
		...
		$res = $query->table("cat_usuarios")->select("id, nickName")->where("nickName", "=", $nickName, "s")->limit(1)->execute();
        ...
        if (!$password1 = validarFormulario('s',$_POST['password1'], 5))
        {
        ...
        if (!$password2 = validarFormulario('s',$_POST['password2'], 5))
        {
            $response['mensaje'] = "El campo Contraseña no puede estar en blanco y debe contener al menos 6 caracteres";
        ...
        $cntrsn 				= $password->hash($password1);
        ...
        //////////////////////////////////////////////////////////////////////////////////////////////////
        $usuario = $idUsuario = $sesion->get('id');
		...
		if ($query->status())
		{
			...
			$query->table("cat_permisos")->insert(compact("idUsuario", "listarContratos", "agregarContrato", "modificarContrato", "eliminarContrato",
