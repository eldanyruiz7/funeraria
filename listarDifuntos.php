				...
				<div class="main-content-inner">
					<div class="breadcrumbs ace-save-state" id="breadcrumbs">
						<ul class="breadcrumb">
							...
							<li class="active">Listar difuntos</li>
						</ul><!-- /.breadcrumb -->
							...
							<div class="modal-body">
								<form class="form-horizontal" role="form" onSubmit="return false">
									<div class="form-group">
										<label class="col-xs-12 text-left"><i class="fa fa-question-circle" aria-hidden="true"></i> ¿Deseas cancelar esta registro del sistema? <br/> Toda la información relacionada será eliminada y no se podrá acceder a ella. Esta acción no puede deshacerse.</label>
									</div>
								</form>
...
<![endif]-->
		...
		<script type="text/javascript">
			...
			{
				console.log(d);
				htmlReturn ='<table class="table table-striped table-bordered table-hover" style="width:100%">'+
							'	<tr>'+
							'		<th>Ver galería</th>'+
							...
							'	</tr>'+
							'	<tr>'+
							'		<td class="text-center">'+d.galeria+'</td>'+
							'		<td>'+d.domicilioPart+'</td>'+
							'		<td>'+d.fechaNac+'</td>'+
							'		<td>'+d.lugarDefuncion+'</td>'+
							'		<td>'+d.noCertificado+'</td>'+
							'		<td>'+d.noActa+'</td>'+
							'		<td>'+d.fechaReg+'</td>'+
							'		<td>'+d.usuario+'</td>'+
							'	</tr>'+
							'</table>'+
							'<div class="divGaleria"></div>';
							console.log(d.galeria);
			...
			$(document).ready(function()
			{
				...
				$(document).on('click','#btnEliminarModal',function()
				{
			        ...
			        {
			            method: "POST",
			            url:"assets/ajax/eliminarDifunto.php",
			        ...
			        .done(function(p)
			        {
						...
						if (p.status == 1)
						{
							...
							{
							 	dialog.find('.bootbox-body').html('<i class="fa fa-check" aria-hidden="true"></i>Este registro ha sido cancelado correctamente');
				...
				$(document).on("click",".aGaleria",function()
				{
					icono = $(this);
					prevHtml = icono.parent().html();
					parent = icono.parent();
					// alert(prevHtml);
					idDifunto = icono.attr("id-difunto");
					icono.parent().html(cargarSpinner);
					$.ajax(
					{
						method: "POST",
						url:"assets/ajax/listarGaleriaDifunto.php",
						data: {idDifunto:idDifunto}
					})
					.always(function()
					{
					...
					.done(function(p)
					{
						esteFancy = parent.parent().parent().parent().parent().find(".divGaleria");
						esteFancy.html(p);
						$("a.imgGrupo[rel=galeria"+idDifunto+"]").fancybox(
						{
							// selector : ,
							helpers : {
						        title: {
			...
			jQuery(function($) {
				...
				myTable = $('#dynamic-table').DataTable( {
			        ...
			        "ajax":
					{
						"url":"assets/ajax/listarDifuntos.JSON.php",
						"type": "POST",
						"complete": function()
						{
				...
				$(document).on('click', 'td.details-control', function ()
				{
					...
					else
					{
						...
						{
							method: "POST",
							url:"assets/ajax/obtenerRowDet_difunto.php",
							dataType:'JSON',
